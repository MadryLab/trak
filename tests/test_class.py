...
@pytest.mark.cuda
def test_score_jvp_basicsingleproj(tmp_path):
    print(trak.__file__)
    ch.manual_seed(0)
...
@pytest.mark.cuda
def test_score_jvp(tmp_path, cpu_proj):
    print(trak.__file__)
    ch.manual_seed(0)
    ch.cuda.manual_seed(42)
    model = resnet18().cuda().eval()
    ...
    projector = BasicProjector(
        grad_dim=11689512, proj_dim=200, seed=0, proj_type="rademacher", device="cuda"
    )
    ...
    assert ch.isclose(scores_jvp.cpu(), scores, atol=1e-5).all()
